...
function showDetalle(eleNum,val){
	if(val==0){
		$(".detalle"+eleNum).hide();
		$(".detalleButton"+eleNum).html("+");
		$(".detalleButton"+eleNum).click(hideDetalle(eleNum,1));	
	}else if(val==1){
		$(".detalle"+eleNum).show();
		$(".detalleButton"+eleNum).html("-");
		$(".detalleButton"+eleNum).click(hideDetalle(eleNum,0));
	}
};
			...
			for(var i=0;i<cant;i++){
				...
				if(i%2==0){	
					...
					if(qresp!=null){
						...
						if(candTxt.length>1){
							candTxt = "<button type=button style='margin-top:4px;border:none' onclick='showDetalle("+canInd+",1)'>+</button><div class=detalle"+canInd+" style='display:none;'>"+candTxt+"</div>";
						}
					...
					if(qresp!=null){
						var candTxt = qresp.answer_text;
						if(candTxt.length>1){
							candTxt = "<button type=button style='margin-top:4px;border:none' onclick='showDetalle("+canInd+")'>+</button><div class=detalle"+canInd+" style='display:none;'>"+candTxt+"</div>";
						}
